...
export const getBlockRangeByBatch = async (
  ...
  const oneOfMatchedBlock = await binarySearchMatchedBlock(
    batchNum,
    latestBlockNum,
    nodeInterface,
    l2Network,
  ...
  console.log(
    `One of matched block founded: ${oneOfMatchedBlock}, now we will go and find all matched blocks...`,
  );
  const targetBlockRange = await findBlockRange(
    batchNum,
    oneOfMatchedBlock,
    latestBlockNum,
    nodeInterface,
